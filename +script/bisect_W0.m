function varargout = bisect_W0(vm,response,varargin)
% relative distance function
fY = @(SOL) 1 - response.map(SOL).response_value/response.prediction;

% set logging function
sLog = struct(...
	'fInit', @(SOL) fprintf('%21s\t%21s\t%21s\t%9s\n','beta0','theta0','W0','chi2'),...
	'fIter', @(SOL) fprintf('%1.15e\t%1.15e\t<strong>%1.15e</strong>\t%1.3e\n',SOL.data.beta0,SOL.data.theta0,SOL.data.W0,response.chi2(SOL)) ...
);
...
[varargout{1:nargout}] = model.tov.rar.bisect.W0(arg);
